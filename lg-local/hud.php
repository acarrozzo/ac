...
<?php 
	if ($infight >= 1) 		{ $hudActive = "inBattle"; }
	else 		{ $hudActive = ""; }
...
if ( $row['hp'] >  $row['hpmax'] ) // HP EXTRA
	...
	echo '<strong class="statBox">';
			...
			if ($weapontype == 3) { echo '<span class="green"><i class="icon-bow-arrow lgray"></i>'.$row['dexmod'].' </span> '; }
	else { echo '<span class="red"><i class="icon-sword lgray"></i>'.$row['strmod'].' </span>';}
...
echo '</div>';	// end uBox
	...
	echo '<div id="eBox">';     // ENEMY BATTLE BOX
		...
		<strong class="buffbound">';
	  	if ($_SESSION['eFly'] >= 1) { echo '<strong class="buffBox blue">Flying</strong>'; }
	  	if ($_SESSION['eCrit'] >= 1) { echo '<strong class="buffBox white">Crit</strong>'; }
	  	if ($_SESSION['eRage'] >= 1) { echo '<strong class="buffBox red">Rage</strong>'; }
	  	if ($_SESSION['ePow'] >= 1) { echo '<strong class="buffBox red">Pow</strong>'; }
	  	if ($_SESSION['eBite'] >= 1) { echo '<strong class="buffBox red">Bite</strong>'; }
	  	if ($_SESSION['eDex'] >= 1) { echo '<strong class="buffBox green">Range</strong>'; }
	  	if ($_SESSION['eMag'] >= 1) { echo '<strong class="buffBox blue">Mag</strong>'; }
	  	if ($_SESSION['eHeal'] >= 1) { echo '<strong class="buffBox green">Heal</strong>'; }
	  	if ($_SESSION['eSteal'] >= 1) { echo '<strong class="buffBox gold">Steal'.$_SESSION['eSteal'].'</strong>'; }
	  	if ($_SESSION['eMulti'] >= 1) { echo '<strong class="buffBox purple">Multi '.$_SESSION['eMulti'].'</strong>'; }
	  	if ($_SESSION['eDoubleHit'] >= 1) { echo '<strong class="buffBox white">2x Hit</strong>'; }
	  	if ($_SESSION['eTripleHit'] >= 1) { echo '<strong class="buffBox white">3x Hit</strong>'; } 
	  	if ($_SESSION['ePureA'] >= 1) { echo '<strong class="buffBox eBox red">Pure A</strong>'; }
	  	if ($_SESSION['ePureD'] >= 1) { echo '<strong class="buffBox eBox gold">Pure D</strong>'; }
	  	if ($_SESSION['eStrImm'] >= 1) { echo '<strong class="buffBox eBox red">Str Imm</strong>'; }
	  	if ($_SESSION['eDexImm'] >= 1) { echo '<strong class="buffBox eBox green">Dex Imm</strong>'; }
	  	if ($_SESSION['eMagImm'] >= 1) { echo '<strong class="buffBox eBox blue">Mag Imm</strong>'; }
	  	if ($_SESSION['eDodge'] >= 1) { echo '<strong class="buffBox eBox green">Dodge '.$_SESSION['eDodge'].'</strong>'; }
	  	if ($_SESSION['ePoison'] >= 1) { echo '<strong class="buffBox eBox green">Poison '.$_SESSION['ePoison'].'</strong>'; }	
		echo '</strong>';
...
$activeMag =""; 
$tabType = "";	
	if ($weapontype == 3) { $tabType = "dex";  }
	if ($magmod >= $strmod) { $activeAtt = ""; $activeMag ="active";  }
  ...
  <div class="battleTabs">';
	...
	if ($weapontype == 3) {	echo '<a href="/" class="btn '.$activeAtt.' '.$tabType.'" data-link="battle-actions"><i class="icon-bow-arrow"></i> Actions</a>'; }
		else { echo '<a href="/" class="btn '.$activeAtt.'" data-link="battle-actions"><i class="icon-crossed-swords"></i> Actions</a>'; }
  ...
  <div class="battleTab '.$activeMag.'" data-pop="battle-magic">';
		...
		if ($ironskin >= 1)  { 
		  ...
		  echo '</div>
  <div class="battleTab" data-pop="battle-bag">BAG</div>';
  	echo '</div>';
