...
while ($row = $result->fetch_assoc()) {
    ...
    if ($input=='grab arrows') {  // ---- GRAB ARROWS
        if ($arrows >= 50) {
            ...
            echo $message="<div class='menuAction'><i class='fa fa-arrow-circle-up green'></i>You grab a bundle of arrows! [ +50 arrows ]</div>";
            include('update_feed.php'); // --- update feed
    ...
    if ($input=='grab bolts') {  // ---- GRAB BOLTS
        if ($bolts >= 50) {
            ...
            echo $message="<div class='menuAction'><i class='fa fa-arrow-circle-up green'></i>You grab a bundle of bolts! [ +50 bolts ]</div>";
            include('update_feed.php'); // --- update feed
    ...
    if ($input=='grab javelins') {  // ---- GRAB ARROWS
        if ($javelin >= 10) {
            ...
            echo $message="<div class='menuAction'><i class='fa fa-arrow-circle-up green'></i>You grab a bundle of javelins! [ +10 javelins ]</div>";
            include('update_feed.php'); // --- update feed
    ...
    if ($input=='grab iron daggers') {  // ---- GRAB ARROWS
        if ($irondagger >= 3) {
            ...
            echo $message="<div class='menuAction'><i class='fa fa-arrow-circle-up green'></i>You grab a bundle of iron daggers! [ +3 iron daggers ]</div>";
            include('update_feed.php'); // --- update feed
    ...
    if ($input=='grab polearm') {
        if ($polearm >= 1) {
            echo $message="<div class='menuAction'><i class='fa fa-times-circle lightred'></i>You already have a polearm. If you lose it come back here for another free one.</div>";
            include('update_feed.php'); // --- update feed
        } else {
            echo $message="<div class='menuAction'><i class='fa fa-arrow-circle-up'></i>You grab a polearm and put in your pack! [ +3 polearm ]</div>";
            include('update_feed.php'); // --- update feed
    ...
    if ($input=='info 40') {
	 ...
	 The Gatekeeper is an exceptionally strong enemy. Arm yourself accordingly and head to the northwest part of the mountains to find the bridge he guards.</div>";
        include('update_feed.php'); // --- update feed
    } elseif ($input=='complete 40') {
